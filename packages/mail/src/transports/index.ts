...
function toAddress(addr: MailAddress | string): string {
...
export class SmtpTransport implements MailTransport {
    ...
    async send(mail: BuiltMail): Promise<void> {
        // nodemailer v7: createTransport is a named export
        ...
        await transporter.sendMail({
            ...(mail.from !== undefined && { from: toAddress(mail.from) }),
            to: mail.to.map(toAddress).join(', '),
            ...(mail.cc !== undefined && { cc: mail.cc.map(toAddress).join(', ') }),
            ...(mail.bcc !== undefined && { bcc: mail.bcc.map(toAddress).join(', ') }),
            ...(mail.replyTo !== undefined && { replyTo: toAddress(mail.replyTo) }),
...
export class SesTransport implements MailTransport {
    ...
    async send(mail: BuiltMail): Promise<void> {
        ...
        await client.send(new aws.SendEmailCommand({
            Source: mail.from ? toAddress(mail.from) : '',
